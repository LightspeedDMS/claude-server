...
public class SwaggerResponseValidationTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    ...
    [Fact]
    public async Task AuthLogin_BadRequestErrorResponse_ShouldHaveConsistentStructure()
    {
        ...
        // Assert - Response should have consistent error structure
        ...
        if (!string.IsNullOrEmpty(loginContent))
        {
            ...
            // Should follow AuthErrorResponse structure (even if not documented in Swagger)
        ...
        // Note: AuthErrorResponse schema is not automatically generated by Swagger
        // because it's returned in catch blocks, not declared in method signatures
        // This test validates actual API behavior even without Swagger documentation
    }
    ...
    public async Task UnauthorizedEndpoint_ShouldReturn401()
    {
        ...
        // Assert - Should return 401 (actual API behavior)
        jobsResponse.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
        
        // Note: The Swagger documentation doesn't automatically document 401 responses
        // unless explicitly declared with ProducesResponseType attributes
        // This test validates actual API behavior vs documented behavior
        ...
        jobsPath.TryGetProperty("responses", out var responses).Should().BeTrue();
        
        // Verify at least 200 response is documented
        responses.TryGetProperty("200", out _).Should().BeTrue("Jobs endpoint should document 200 response");
    }
    ...
    [Fact]
    public async Task SwaggerJson_ShouldHaveBasicEndpointDocumentation()
    {
        // Act
        var swaggerResponse = await _client.GetAsync("/swagger/v1/swagger.json");
        var jsonContent = await swaggerResponse.Content.ReadAsStringAsync();
        ...
        // Assert - All endpoints should have at least basic documentation
        ...
        foreach (var pathItem in paths.EnumerateObject())
        {
            foreach (var operation in pathItem.Value.EnumerateObject())
            {
                if (operation.Value.ValueKind == JsonValueKind.Object)
                {
                    ...
                    {
                        endpointsWithoutResponses.Add($"{operation.Name.ToUpper()} {pathItem.Name}");
                    }
                    else
                    {
                        // Should have at least a 200 response documented
                        responses.TryGetProperty("200", out _).Should().BeTrue(
                            $"Endpoint {operation.Name.ToUpper()} {pathItem.Name} should document 200 response");
                    }
                }
            }
        }
        ...
        endpointsWithoutResponses.Should().BeEmpty(
            $"All endpoints should have responses. Missing: {string.Join(", ", endpointsWithoutResponses)}");
        
        // Note: Error responses (500, 401, etc.) are not automatically documented unless
        // explicitly declared with ProducesResponseType attributes in the controllers
    }
